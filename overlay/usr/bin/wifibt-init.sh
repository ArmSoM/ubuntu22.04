...
WIFIBT_MODULE_DIR="/usr/lib/modules"

do_insmod()
{
	if ! lsmod | grep -wq "$1"; then
		echo "Installing $1.ko ..."
		KO_PATH=$(find $WIFIBT_MODULE_DIR -type f -name "$1.ko")
		if [ "$1" = "rk960" ]; then
			insmod "$KO_PATH" fw_no_sleep=1
		else
			insmod "$KO_PATH"
		fi
		sleep "${2:-0}"
...
do_start_bt()
{
	...
	case "$WIFIBT_VENDOR" in
		...
		aic8800)
			case "$WIFIBT_BUS" in
				usb) start_bt_aic_usb;;
				sdio) start_bt_aic_sdio;;
			esac
			;;
		*)
